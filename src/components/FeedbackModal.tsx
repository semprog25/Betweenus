...
export function FeedbackModal({ isOpen, onClose }: FeedbackModalProps) {
  ...
  const ratingReactions = [
    { stars: 1, message: t('feedback.rating.1.msg'), color: 'text-red-500', description: t('feedback.rating.1.desc') },
    { stars: 2, message: t('feedback.rating.2.msg'), color: 'text-orange-500', description: t('feedback.rating.2.desc') },
    { stars: 3, message: t('feedback.rating.3.msg'), color: 'text-yellow-500', description: t('feedback.rating.3.desc') },
    { stars: 4, message: t('feedback.rating.4.msg'), color: 'text-blue-500', description: t('feedback.rating.4.desc') },
    { stars: 5, message: t('feedback.rating.5.msg'), color: 'text-purple-500', description: t('feedback.rating.5.desc') },
  ];
          ...
          <div className="space-y-3 py-2">
            {feedbackTypes.map((type, index) => {
              ...
              <motion.button
                ...
                className="w-full p-4 rounded-xl border-2 border-purple-200 dark:border-purple-500/30 hover:border-purple-400 dark:hover:border-purple-400 bg-white dark:bg-gray-800/50 hover:bg-purple-50 dark:hover:bg-purple-950/30 transition-all group text-left min-h-[72px]"
              >
                <div className="flex items-center gap-3">
                  <div className={`w-12 h-12 rounded-lg ${type.bgColor} flex items-center justify-center`}>
                    <TypeIcon className="w-6 h-6 text-purple-700 dark:text-purple-300" strokeWidth={2} />
                  </div>
                  <div className="flex-1">
                    <h4 className="font-semibold text-gray-900 dark:text-gray-100">{type.label}</h4>
                    <p className="text-sm text-gray-600 dark:text-gray-400">{type.description}</p>
                  </div>
                  <span className="text-gray-400 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors" aria-hidden="true">→</span>
                </div>
              </motion.button>
              )
            })}
                    ...
                    >
                      <div className="flex gap-1">
                        {Array.from({ length: rating }).map((_, i) => (
                          <Star key={i} className={`w-5 h-5 fill-current ${ratingReactions[rating - 1].color}`} />
                        ))}
                      </div>
                    </motion.div>
                    ...
                    >
                      <div className="bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl p-4 text-white space-y-3">
                        <div className="text-center">
                          ...
                          >
                            <Star className="w-10 h-10 text-yellow-300 fill-yellow-300" strokeWidth={1.5} />
                          </motion.div>
