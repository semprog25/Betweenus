...
import { Loader2, Settings, Mail, Lock, Eye, EyeOff, Trash2 } from 'lucide-react';
import { toast } from 'sonner@2.0.3';
import { callServer } from '../utils/supabase/client';
import { getSession, deleteAccount, signOut } from '../utils/auth';
...
export function SettingsModal({ isOpen, onClose, onAccountDeleted }: SettingsModalProps) {
  const { t } = useLanguage();
  ...
  const handleDeleteAccount = async () => {
    if (deleteConfirmText !== 'DELETE') {
      toast.error(t('settings.deleteConfirmType') || 'Type DELETE to confirm');
      return;
    }
    ...
    try {
      await deleteAccount();
      await signOut();
      toast.success(t('profile.deleteSuccess') || 'Your account has been deleted');
      ...
      toast.error(error.message || t('profile.deleteError') || 'Failed to delete account');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-md bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-900 dark:to-purple-950/30 border-2 border-purple-200 dark:border-purple-500/50">
        ...
        <div className="flex gap-2 p-1 bg-white/50 dark:bg-gray-800/50 rounded-xl">
          <button
            ...
            className={`flex-1 px-3 py-2 rounded-lg transition-all text-sm min-h-[44px] ${
              activeTab === 'email'
          ...
          >
            ...
            {t('settings.tab.account') || 'Account'}
          </button>
        </div>
        ...
        {activeTab === 'account' && (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {t('settings.deleteDescription') || 'Permanently delete your account and personal profile data. Anonymous community posts may remain untraceable.'}
            </p>
            <Button
              variant="destructive"
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t('profile.deleteAccount') || 'Delete account'}</AlertDialogTitle>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isLoading}>{t('profile.deletePost.cancel')}</AlertDialogCancel>
            <AlertDialogAction
              onClick={(e) => {
                e.preventDefault();
                handleDeleteAccount();
            ...
            >
              {isLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : (t('profile.deletePost.confirm') || 'Delete')}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
