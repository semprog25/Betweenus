...
import { Globe, Camera, Edit2, Check, LogOut, User as UserIcon, Loader2, Heart, MessageCircle, MessageSquare, ThumbsUp, Calendar, Trash2, Edit, Star, Crown, Settings, Shield, FileText, HelpCircle, Info, Award, TrendingUp, Zap, Target, Medal, Trophy, Handshake, PartyPopper, Sprout, Gem, BookOpen, ScrollText } from 'lucide-react';
...
import { getMoodByEmoji, MoodIconBadge } from './mood/mood-visuals';
  ...
  const getAchievements = () => {
    ...
    if (levelData?.badges && levelData.badges.length > 0) {
      return levelData.badges.map((badge: any) => {
        ...
        return {
          id: badge.id,
          icon: resolveBadgeIcon(badge.id, badge.icon),
          title: title,
          description: desc,
    ...
    <div className="h-full overflow-y-auto scrollbar-hide">
      <div className="max-w-4xl mx-auto p-6 space-y-6 pb-8">
        ...
        {!userSession && (
          ...
          >
            <p className="text-purple-600 dark:text-purple-300 text-sm flex items-center justify-center gap-2">
              <Info className="w-5 h-5 shrink-0" />
              <span>{t('profile.anonymous')}
              <br />
              <a href="#" onClick={(e) => { e.preventDefault(); window.location.reload(); }} className="underline font-medium">
                {t('profile.createAccount')}
              </a> {t('profile.sync')}</span>
            </p>
          </motion.div>
        )}
              ...
              {checkInHistory.map((checkIn, idx) => (
                ...
                >
                  <div className="flex items-center justify-center">
                    {(() => {
                      const mood = getMoodByEmoji(checkIn.emoji || '')
                      if (mood) return <MoodIconBadge mood={mood} size="sm" />
                      return (
                        <div
                          className="w-10 h-10 rounded-2xl flex items-center justify-center shadow-sm border-2 border-white dark:border-gray-700"
                ...
                >
                  <div className="mb-2 flex justify-center">
                    {(() => {
                      ...
                      return <IconComp className="w-8 h-8 text-purple-700 dark:text-purple-300" strokeWidth={2} />
                    })()}
                  </div>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                  <HelpCircle className="w-4 h-4 text-blue-600 dark:text-blue-400" strokeWidth={2} />
                </div>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                  <MessageSquare className="w-4 h-4 text-green-600 dark:text-green-400" strokeWidth={2} />
                </div>
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-orange-100 dark:bg-orange-900/30 flex items-center justify-center">
                  <ScrollText className="w-4 h-4 text-orange-600 dark:text-orange-400" strokeWidth={2} />
                </div>
