...
import { Heart, MessageCircle, Share2, Bookmark, Search, TrendingUp, Clock, Filter, Sparkles, ThumbsUp, ThumbsDown, RefreshCw, X } from 'lucide-react';
...
export function CommunityTab() {
  ...
  const [userId, setUserId] = useState('anonymous-user-' + Math.random().toString(36).substr(2, 9));

  // Initialize User ID and listen for auth changes
  useEffect(() => {
    const checkSession = () => {
      ...
      if (session?.user?.id) {
        ...
        // Fallback to existing anonymous ID if logged out
        const savedAnonId = localStorage.getItem('between_us_anon_id');
        if (savedAnonId) {
          setUserId(savedAnonId);
        } else {
          // Keep the random one generated in state, but save it
          localStorage.setItem('between_us_anon_id', userId);
        }
      }
    };
    ...
    // Listen for storage changes (login/logout events from other tabs/components)
    window.addEventListener('storage', checkSession);
    // Also listen for custom auth events if any (or just poll/re-check on focus)
  ...
  const categories = [
    { id: 'All', emoji: '' }, 
    { id: 'Controversial', emoji: '🔥' },
    { id: 'Clickbait', emoji: '💣' },
    { id: 'Exposed', emoji: '🚨' },
    { id: 'Heartbreak', emoji: '💔' },
    { id: 'Shocking', emoji: '😱' },
    { id: 'Confessions', emoji: '🤯' },
    { id: 'Dark Secrets', emoji: '💀' },
    { id: 'Drama', emoji: '🎭' },
    { id: 'Tea & Gossip', emoji: '👀' },
    { id: 'Money Problems', emoji: '💸' },
    { id: 'NSFW Stories', emoji: '🔞' },
    { id: 'Unpopular Opinions', emoji: '⚠️' },
    { id: 'Addictions', emoji: '❤️‍🩹' },
    ...
    { id: 'Education', emoji: '🎓' },
    { id: 'Self-Care', emoji: '💪' },
    { id: 'Personal Growth', emoji: '🌟' },
    { id: 'Anxiety', emoji: '😔' },
    { id: 'Depression', emoji: '💭' },
    { id: 'Friendships', emoji: '🤝' },
    { id: 'Motivation', emoji: '✨' },
    { id: 'Random', emoji: '🎲' }
  ];
  ...
  const getPosts = async (language: string) => {
    ...
    const response = await fetch(`${baseUrl}/posts?language=${language}`, {
      headers: {
  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      console.log(`Loaded community posts for language ${language}:`, postsData.length);
  ...
  const refreshPosts = async () => {
    ...
    try {
      // Fetch posts based on current sort filter
      const baseUrl = `https://${projectId}.supabase.co/functions/v1/make-server-6c9b0e48`;
      const response = await fetch(`${baseUrl}/posts?language=${language}&sort=${sortBy}`, {
        headers: {
          'Authorization': `Bearer ${publicAnonKey}`,
        },
      });
                    ...
                    {post.categories && post.categories.length > 0 && (
                      <div className="flex flex-wrap gap-2 mt-3">
                        {post.categories.map((categoryName, idx) => {
                          const categoryDef = categories.find(c => c.id === categoryName);
                          const emoji = categoryDef?.emoji || '💬';
                          
                          // Determine if this is a "hot" category for special styling
