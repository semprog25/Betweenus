...
import { CalendarIcon, BookOpen, TrendingUp, Sparkles, ChevronLeft, ChevronRight, User, Flame, Trophy, Quote, Check, Footprints, Brain, LineChart, Lightbulb, Sprout, Handshake, Heart, MessageCircle } from 'lucide-react';
import { getSession } from '../utils/auth';
import { saveCheckIn, getCheckIns } from '../utils/api';
import { ProfilePictureUpload } from './ProfilePictureUpload';
import { useLanguage } from './LanguageContext';
import { toast } from 'sonner@2.0.3';
import { MAIN_MOODS, MoodIconBadge, type MoodVisual } from './mood/mood-visuals';
import { ACTIVITIES, getActivityKey } from './mood/activity-visuals';
...
const findMoodByEntry = (entry: MoodEntry): MoodVisual | undefined =>
  MAIN_MOODS.find((m) => m.name === entry.mainMood) ||
  MAIN_MOODS.find((m) => m.emoji === entry.emoji)
...
export function CheckInTab({ userName = 'Friend', profilePicture: propProfilePicture = '', onNavigateToProfile }: CheckInTabProps) {
  ...
  const handleMainMoodSelect = (mood: MoodVisual) => {
  ...
  const getMoodForDate = (date: Date): MoodVisual | null => {
    const entries = getEntriesForDate(date);
    if (entries.length === 0) return null;
    const latest = entries[entries.length - 1];
    return findMoodByEntry(latest) || null;
  };
          ...
          <div className="flex-shrink-0 pt-4 pb-4">
            ...
            >
              <div className="flex items-start gap-2">
                <div className="w-9 h-9 rounded-xl bg-purple-100 dark:bg-purple-900/40 flex items-center justify-center shrink-0" aria-hidden="true">
                  <MessageCircle className="w-5 h-5 text-purple-600 dark:text-purple-300" strokeWidth={2} />
                </div>
          ...
          <div className="flex-1 overflow-y-auto pb-4 space-y-4 scrollbar-hide">
            ...
            >
              <Card className="bg-white/90 dark:bg-gray-800/90 backdrop-blur-sm p-4 rounded-3xl shadow-xl border-2 border-purple-100 dark:border-purple-900">
                ...
                >
                  <Sparkles className="w-5 h-5 text-purple-500 dark:text-purple-400" strokeWidth={2} aria-hidden="true" />
                  {t('checkin.greeting')}
                </motion.h3>
            ...
            <div className="text-base pt-4 space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2 text-sm text-muted-foreground">
                    ...
                    <li className="flex items-start gap-2">
                      <Trophy className="w-4 h-4 text-yellow-500 mt-0.5 shrink-0" aria-hidden="true" />
                      <span><strong>{t('checkin.rule2').split(': ')[0]}:</strong> {t('checkin.rule2').split(': ')[1]}</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <Flame className="w-4 h-4 text-orange-500 mt-0.5 shrink-0" aria-hidden="true" />
                      <span><strong>{t('checkin.rule3').split(': ')[0]}:</strong> {t('checkin.rule3').split(': ')[1]}</span>
                    </li>
                    <li className="flex items-start gap-2">
                      <Heart className="w-4 h-4 text-purple-500 mt-0.5 shrink-0" aria-hidden="true" />
            ...
            <div className="text-base pt-4 space-y-4">
                <div className="space-y-3">
                  ...
                  <ul className="space-y-2.5 text-sm text-muted-foreground">
                    <li className="flex items-start gap-2">
                      <Brain className="w-5 h-5 text-purple-500 mt-0.5 shrink-0" aria-hidden="true" />
                      <div>
                    ...
                    <li className="flex items-start gap-2">
                      <LineChart className="w-5 h-5 text-blue-500 mt-0.5 shrink-0" aria-hidden="true" />
                      <div>
                    ...
                    <li className="flex items-start gap-2">
                      <Sprout className="w-5 h-5 text-emerald-500 mt-0.5 shrink-0" aria-hidden="true" />
                      <div>
                    ...
                    <li className="flex items-start gap-2">
                      <Handshake className="w-5 h-5 text-cyan-500 mt-0.5 shrink-0" aria-hidden="true" />
                      <div>
          ...
          <div className="flex-1 flex items-center overflow-hidden pb-6">
            <div className="w-full grid grid-cols-2 gap-2.5 auto-rows-fr">
              {MAIN_MOODS.map((mood, index) => (
                ...
                >
                  <MoodIconBadge mood={mood} size="lg" />
        ...
        <div className="h-full max-w-md mx-auto px-4 sm:px-6 flex flex-col">
          <div className="flex-shrink-0 pt-4 pb-8">
            ...
            <div className="flex flex-col items-center gap-3">
              <MoodIconBadge mood={selectedMainMood} size="lg" />
          ...
          <div className="flex-1 overflow-y-auto pb-24 space-y-6 scrollbar-hide pt-2">
            ...
            >
              <h3 className="text-sm font-semibold text-foreground/80 mb-3 px-1 flex items-center gap-2">
                <Footprints className="w-4 h-4 text-purple-500" aria-hidden="true" />
                {t('checkin.activities')}
              </h3>
                ...
                {ACTIVITIES.map((activity, idx) => {
                  ...
                  return (
                    <motion.button
                      ...
                      aria-label={t(`activity.${activityKey}`)}
                      className={`
                        relative flex flex-col items-center justify-center gap-1 p-2.5 rounded-xl text-xs font-medium transition-all duration-200 border min-h-[72px]
                        ${isSelected 
                      ...
                      `}
                    >
                      <ActivityIcon className={`w-5 h-5 mb-0.5 ${isSelected ? 'text-purple-600 dark:text-purple-300' : 'text-muted-foreground'}`} strokeWidth={2} />
            ...
            <div className="flex-1 overflow-y-auto pb-6 px-4 sm:px-6 scrollbar-hide">
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  const moodVisual = hasEntries ? getMoodForDate(date) : null;
                  ...
                  return (
                    <motion.button
                      ...
                      whileTap={{ scale: 0.95 }}
                      aria-label={`${day}${hasEntries ? `, ${entries[entries.length - 1].mainMood}` : ''}`}
                      className="aspect-square flex flex-col items-center justify-center relative min-h-[44px]"
                    >
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-full flex items-center justify-center ${
                        ...
                        {moodVisual ? (
                          <MoodIconBadge mood={moodVisual} size="sm" />
                        ) : (
          ...
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {selectedDate && getEntriesForDate(selectedDate).map((entry) => {
                const moodVisual = findMoodByEntry(entry)
                return (
                <Card key={entry.id} className={`${moodVisual?.bg || 'bg-card'} ${moodVisual?.border || 'border-border'} border p-4`}>
                  <div className="flex items-start gap-3">
                    {moodVisual ? <MoodIconBadge mood={moodVisual} size="sm" /> : null}
                    <div className="flex-1">
                            ...
                            >
                              <div className="flex items-start gap-3">
                                ...
                                {(() => {
                                  const moodVisual = findMoodByEntry(entry)
                                  return moodVisual ? (
                                    <MoodIconBadge mood={moodVisual} size="sm" className="flex-shrink-0" />
                                  ) : (
                                    <div className={`w-12 h-12 rounded-full ${moodBgColor} flex-shrink-0`} />
                                  )
                                })()}
