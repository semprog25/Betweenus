...
export function AuthStep({ userName, selectedLanguages, onComplete, onBack }: AuthStepProps) {
  ...
  const handleGoogleSignIn = async () => {
    if (import.meta.env.DEV && !oauthSetupWarningShown) {
      toast.info(t('auth.googleSetup'), { duration: 5000 });
  ...
  const handleAppleSignIn = async () => {
    ...
    if (isNativeMobile() || isWeb()) {
      ...
      try {
        ...
        toast.error(t('auth.appleFailed') || 'Apple sign-in is unavailable. Try email or Google instead.');
      } finally {
        setIsLoading(false);
      }
      return;
      ...
      <div className="bg-[#0f0f1e]/90 rounded-3xl p-6 backdrop-blur-sm border border-white/10">
        ...
        <div className="space-y-2 mb-4">
          <Button
            ...
            disabled={isLoading}
            variant="outline"
            className="w-full bg-white hover:bg-gray-100 text-gray-900 border-gray-300 h-11 min-h-[44px]"
          >
            <GoogleIcon />
          ...
          <Button
            ...
            disabled={isLoading}
            variant="outline"
            className="w-full bg-black hover:bg-gray-900 text-white border-gray-700 h-11 min-h-[44px]"
          >
            <AppleIcon />
