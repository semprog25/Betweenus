...
app.post("/make-server-6c9b0e48/check-ins", async (c) => {
  try {
    ...
    // Also save by date for easy calendar lookup
    const dateKey = new Date(checkIn.date).toISOString().split('T')[0];
    await kv.set(`check-in-by-date:${dateKey}`, checkIn);

    console.log(`Check-in saved: ${checkInId}`);
...
app.get("/make-server-6c9b0e48/check-ins", async (c) => {
  try {
    ...
    if (startDate && endDate) {
      ...
      const checkIns = await kv.getByPrefix("check-in-by-date:");
...
app.post("/make-server-6c9b0e48/posts", async (c) => {
  try {
    const body = await c.req.json();
    const { content, mood, isAnonymous, languages, userId, categories } = body;
    ...
    const post = {
      ...
      userId: userId || null, // Track which user created it
      upvotes: 0,
      downvotes: 0,
    ...
    if (userId) {
      await kv.set(`user-post:${userId}:${postId}`, post);
    }

    console.log(`Post created: ${postId}${userId ? ` by user: ${userId}` : ''} with categories: ${categories?.join(', ')}`);
...
// Get all posts (for Listen & Support tab)
  ...
  try {
    ...
    if (language && language !== 'all') {
      filteredPosts = filteredPosts.filter((post: any) => 
        post.languages && post.languages.includes(language)
      );
    }
...
// Update post metadata (for migration purposes - no time restrictions)
...
// Update post likes (for seeding/testing purposes)
...
// Bulk update votes for a post (for seeding/testing)
...
// Bulk update votes for a reply (for seeding/testing)
...
// Get all waitlist emails (admin only - for analytics)
...
app.post("/make-server-6c9b0e48/subscription/buy-credits", async (c) => {
  try {
    const body = await c.req.json();
    const { userId, amount } = body; // amount of credits to buy
...
// DEVELOPER TOOL: Clear all public usernames
